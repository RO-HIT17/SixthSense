...
# ADB command to take screenshot and pull it to PC
def take_screenshot():
    os.system("adb shell screencap -p /sdcard/screen.png")
    os.system("adb pull /sdcard/screen.png")

# Detect unread message positions using OCR
def get_unread_chat_positions(image_path="screen.png"):
    image = cv2.imread(image_path)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    
    # Detect green badge areas (WhatsApp unread messages)
    ...
    for cnt in contours:
        ...
        if 20 < w < 100 and 20 < h < 100:  # Filter out small noise
            unread_positions.append((x, y))

    return unread_positions

# Tap on coordinates (to open chat)
def tap(x, y):
    os.system(f"adb shell input tap {x} {y}")

# Swipe up inside a chat (for longer messages)
def swipe_up():
    os.system("adb shell input swipe 500 1500 500 500")

# Read the last message from chat screen using OCR
...
# Region definitions for text extraction
CHAT_NAME_REGION = (100, 100, 400, 150)  # Adjust these coordinates based on your screen
LAST_MSG_REGION = (100, 150, 400, 200)   # Adjust these coordinates based on your screen

def extract_chat_name(image):
    # Extract chat name from the top bar region
...
def extract_last_message(image):
    # Extract last message from the message region
...
# Main loop: Find unread chats, open them, read messages
